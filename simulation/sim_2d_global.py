"""
Section 5.2 - 2D Global Scenarios (Figure 3, Table 3).

Evaluates FMMT power and Type I error for two global 2D alternatives.
n=200, sigma=0.1, c in [-0.5, 0.5] step 0.05, 1000 replications.

Outputs:
    results/step3_2d_global.json
    results/tables/step3_type_i_error.csv    (Table 3 in paper)
    results/figures/step3_power_curves.png   (Figure 3 in paper)

Usage:
    python -m simulation.sim_2d_global
    python -m simulation.sim_2d_global --quick
"""
import sys
import json
import argparse
...
def run_single_trial(scenario, c):
    try:
        return compute_2d_pvalue_kde(
            n_samples=N_SAMPLES,
            subdomain_modulations=(True, True, True, True),
...
def run_sim_2d_global(n_reps=1000, seed=42, output_dir="results"):
    ...
    json_path = Path(output_dir) / "step3_2d_global.json"
    with open(json_path, "w") as fh:
        json.dump(results, fh, indent=2)
    print(f"\nStep 3 complete. Raw results: {json_path}")
    return results


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Reproduce Figure 3 and Table 3")
    parser.add_argument("--quick", action="store_true")
    parser.add_argument("--n-reps", type=int, default=1000)
    parser.add_argument("--seed",   type=int, default=42)
    args = parser.parse_args()
    run_sim_2d_global(n_reps=50 if args.quick else args.n_reps, seed=args.seed)
