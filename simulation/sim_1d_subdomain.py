"""
Section 5.2 - 1D Subdomain Scenarios (Figure 2).

Tests FMMT sensitivity to localized 1D alternatives.
n=50, sigma=0.1, c in [-2, 2] step 0.2, 1000 replications.

Outputs:
    results/step2_1d_subdomain.json           (raw MC results)
    results/tables/step2_type_i_error.csv
    results/figures/step2_power_curves.png    (Figure 2 in paper)

Usage:
    python -m simulation.sim_1d_subdomain
    python -m simulation.sim_1d_subdomain --quick
"""
import sys
import json
import argparse
...
def run_sim_1d_subdomain(n_reps=1000, seed=42, output_dir="results"):
    ...
    json_path = Path(output_dir) / "step2_1d_subdomain.json"
    with open(json_path, "w") as fh:
        json.dump(results, fh, indent=2)
    print(f"\nStep 2 complete. Raw results: {json_path}")
    return results
...
if __name__ == "__main__":
    ...
    run_sim_1d_subdomain(n_reps=50 if args.quick else args.n_reps, seed=args.seed)
